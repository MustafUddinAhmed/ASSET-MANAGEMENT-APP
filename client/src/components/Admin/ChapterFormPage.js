...
class ChapterFormPage extends Component {
    ...
    if (this.state.chapterName === "") {
      axios.post(`${SERVICE_API_URL}/api/chapters`, chapter).then(response => {
        ...
        if (response.data.success == false) {
          this.setState({
            message: "Chapter Name already exists, Enter a different name"
          });
        } else {
          this.props.history.push(`/api/chapters/get_all`);
        }
  ...
  validateChapterForm(values) {
    ...
    if (!values.chapterName) {
      ...
      errors.chapterDescription = "Enter a Chapter Description";
    }
    return errors;
  }
  render() {
    ...
    return (
      <div>
        ...
        <div className="container myPadding">
          <h1 id="myHeaderClass">{this.state.buttonName} Chapter</h1>
          <Formik
          ...
          >
            <Form>
              ...
              <fieldset className="form-group">
                ...
                <Field
                  className="form-control"
                  placeholder="Enter a Chapter Description"
                  name="chapterDescription"
                  component="textarea"
                  rows="6"
                />
